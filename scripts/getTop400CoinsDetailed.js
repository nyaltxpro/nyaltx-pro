...
async function fetchCoinDetails(coinId, index, total) {
  try {
    ...
    const detailedCoin = {
      ...
      platforms:
        data.platforms && Object.keys(data.platforms).length > 0 && data.platforms[''] !== ''
          ? data.platforms
          : {},
      detail_platforms:
        data.detail_platforms &&
        Object.keys(data.detail_platforms).length > 0 &&
        !data.detail_platforms['']
          ? data.detail_platforms
          : {},
...
async function getTop400CoinsDetailed() {
  ...
  try {
    ...
    console.log(
      `⏱️ Rate limiting: ${RATE_LIMIT_DELAY / 1000}s between requests, ${BATCH_DELAY / 1000}s between batches\n`
    );
    ...
    for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
      ...
      console.log(
        `📦 Processing batch ${batchIndex + 1}/${totalBatches} (coins ${startIdx + 1}-${endIdx})`
      );
      ...
      const batchWithContracts = detailedCoins
        .slice(startIdx)
        .filter(coin => coin.platforms && Object.keys(coin.platforms).length > 0);
      console.log(
        `   ✅ Batch ${batchIndex + 1} complete: ${batchWithContracts.length}/${batch.length} coins have contract addresses`
      );
    ...
    const finalData = {
      metadata: {
        ...
        description:
          'Top 400 cryptocurrencies with detailed information including contract addresses, descriptions, and links',
        data_includes: [
          'contract_addresses',
          'platforms',
          'descriptions',
    ...
    // Statistics
    const coinsWithContracts = detailedCoins.filter(
      coin => coin.platforms && Object.keys(coin.platforms).length > 0
    );
    ...
    const coinsWithDescriptions = detailedCoins.filter(
      coin => coin.description && coin.description.length > 0
    );

    const coinsWithLinks = detailedCoins.filter(
      coin =>
        coin.links &&
    ...
    console.log(
      `   • Native blockchain tokens: ${nativeTokens.length} (${Math.round((nativeTokens.length / detailedCoins.length) * 100)}%)`
    );
    console.log(
      `   • Contract-based tokens: ${contractTokens.length} (${Math.round((contractTokens.length / detailedCoins.length) * 100)}%)`
    );
    console.log(
      `   • Coins with contract addresses: ${coinsWithContracts.length} (${Math.round((coinsWithContracts.length / detailedCoins.length) * 100)}%)`
    );
    console.log(
      `   • Coins with descriptions: ${coinsWithDescriptions.length} (${Math.round((coinsWithDescriptions.length / detailedCoins.length) * 100)}%)`
    );
    console.log(
      `   • Coins with social links: ${coinsWithLinks.length} (${Math.round((coinsWithLinks.length / detailedCoins.length) * 100)}%)`
    );
    console.log(`   • Total execution time: ${Math.floor(duration / 60)}m ${duration % 60}s`);
    console.log(
      `   • Average time per coin: ${Math.round((duration / detailedCoins.length) * 100) / 100} seconds`
    );
    console.log(`   • API calls made: ~${detailedCoins.length + 2} calls`);
    console.log(`   • Output file: ${outputPath}`);
    console.log(
      `   • File size: ${Math.round((fs.statSync(outputPath).size / 1024 / 1024) * 100) / 100} MB`
    );
    ...
    detailedCoins.slice(0, 3).forEach((coin, idx) => {
      ...
      if (coin.categories && coin.categories.length > 0) {
        console.log(
          `      • Categories: ${coin.categories.slice(0, 3).join(', ')}${coin.categories.length > 3 ? '...' : ''}`
        );
      }
    });
