...
async function fetchTop400Coins() {
  ...
  try {
    ...
    for (let i = 0; i < allCoins.length; i += BATCH_SIZE) {
      ...
      console.log(
        `📦 Processing batch ${batchNumber}/${totalBatches} (coins ${i + 1}-${Math.min(i + BATCH_SIZE, allCoins.length)})`
      );
    ...
    const coinsWithAddresses = processedCoins.filter(
      coin => coin.contract_addresses && Object.keys(coin.contract_addresses).length > 0
    );
    ...
    console.log(
      `   • API calls made: ~${processedCoins.length + 2} (2 market calls + ${processedCoins.length} detail calls)`
    );
    console.log(
      `   • Success rate: ${Math.round((coinsWithAddresses.length / processedCoins.length) * 100)}%`
    );
