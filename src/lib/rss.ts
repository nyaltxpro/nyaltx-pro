...
export function parseItemId(
  id: string
): { source: string; title: string; link: string; dateIso: string } | null {
  try {
...
export function parseRss(xml: string, source: NewsSource): RssItem[] {
  ...
  const entries = items.length
    ? []
    : (Array.from(doc.querySelectorAll('feed > entry')) as Element[]);
  ...
  const results: RssItem[] = list.map((el: Element, idx: number) => {
    ...
    const link =
      getAttr(el.querySelector('link'), 'href') || textContent(el.querySelector('link')) || '';
    const pubDateText =
