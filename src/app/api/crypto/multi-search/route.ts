...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(
      `📊 Final results: ${finalResults.length} tokens from ${new Set(finalResults.map(r => r.source)).size} sources`
    );

    // Log success rate for monitoring
    const tokensWithAddresses = finalResults.filter(
      token => token.contractAddresses && Object.keys(token.contractAddresses).length > 0
    );
    console.log(
      `📈 Contract address success rate: ${tokensWithAddresses.length}/${finalResults.length} (${Math.round((tokensWithAddresses.length / finalResults.length) * 100)}%)`
    );
