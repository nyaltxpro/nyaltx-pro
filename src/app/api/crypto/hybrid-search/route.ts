...
export async function GET(request: NextRequest) {
  try {
      ...
      for (let attempt = 0; attempt <= retries; attempt++) {
        try {
          ...
          if (response.ok) {
            ...
            console.log(
              `✅ CoinGecko search results: ${data.coins?.length || 0} coins found for "${query}"`
            );
            return data.coins || [];
          ...
          if (error.name === 'AbortError') {
            ...
            console.error(
              `❌ CoinGecko search error, attempt ${attempt + 1}/${retries + 1}:`,
              error.message
            );
          }
      ...
      for (let attempt = 0; attempt <= retries; attempt++) {
        try {
          ...
          if (error.name === 'AbortError') {
            ...
            console.log(
              `Contract fetch error for ${coin.symbol}, attempt ${attempt + 1}:`,
              error.message
            );
          }
    ...
    const coinsWithAddresses = results.filter(
      coin => coin.contractAddresses && Object.keys(coin.contractAddresses).length > 0
    );
    console.log(
      `📊 Contract addresses retrieved for ${coinsWithAddresses.length}/${results.length} coins`
    );
