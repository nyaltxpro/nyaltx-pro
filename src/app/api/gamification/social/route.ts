...
export async function GET(req: NextRequest) {
  try {
    ...
    const status = validStatuses.includes(statusParam as any)
      ? (statusParam as 'pending' | 'sent' | 'failed')
      : 'pending';
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const pendingAnnouncements = await socialCollection
      .find({
        status: 'pending',
        scheduledAt: { $lte: now },
      })
    ...
    for (const announcement of pendingAnnouncements) {
      try {
        ...
        if (success) {
          ...
          await socialCollection.updateOne({ id: announcement.id }, { $set: { status: 'failed' } });
