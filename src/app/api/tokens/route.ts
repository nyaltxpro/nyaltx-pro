...
async function fetchLocalTokens(params: TokenSearchParams): Promise<NormalizedToken[]> {
  try {
    ...
    const mappedTokens = tokens.map((token: any) => ({
      ...
      marketCap: token.marketCap || token.marketCapUsd || undefined,
      liquidity: token.liquidity || token.liquidityUsd || undefined,
      createdAt: token.createdAt || new Date().toISOString(),
      source: 'local' as const,
      status: token.status || 'pending',
      volume: token.volume || token.volume24h || undefined,
      volume24h: token.volume24h || undefined,
      holders: token.holders || undefined,
      transactions: token.transactions || undefined,
      socials: {
