...
export default function LiveStream({ tokenAddress, className = '' }: LiveStreamProps) {
  ...
  const generateMockActivity = (): StreamActivity => {
    ...
    const mockTokens = [
      {
        ...
        name: 'Pepe Coin',
        address: '0x123...',
        image: '/crypto-icons/color/pepe.svg',
      },
      {
        symbol: 'DOGE',
        name: 'Dogecoin',
        address: '0x456...',
        image: '/crypto-icons/color/doge.svg',
      },
      {
        symbol: 'SHIB',
        name: 'Shiba Inu',
        address: '0x789...',
        image: '/crypto-icons/color/shib.svg',
      },
      {
        symbol: 'FLOKI',
        name: 'Floki Inu',
        address: '0xabc...',
        image: '/crypto-icons/color/floki.svg',
      },
    ];
    ...
    return {
      ...
      message:
        type === 'comment' ? messages[Math.floor(Math.random() * messages.length)] : undefined,
      milestone:
        type === 'milestone'
  ...
  const getActivityIcon = (type: StreamActivity['type']) => {
    switch (type) {
      ...
      case 'sell':
        return <FaDollarSign className="text-red-400" />;
      case 'create':
        return <FaFire className="text-purple-400" />;
      case 'comment':
        return <FaComment className="text-blue-400" />;
      case 'milestone':
        return <FaHeart className="text-yellow-400" />;
    }
  };
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-4">
        <div className="flex items-center justify-between">
          ...
          <div className="text-gray-300 text-sm">🔥 {activities.length} activities today</div>
        </div>
      </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-gray-300 text-sm">
                    {activity.type === 'buy' && (
                      <span>
                        Bought{' '}
                        <span className="text-green-400">{formatNumber(activity.amount!)}</span>{' '}
                    ...
                    {activity.type === 'sell' && (
                      <span>
                        Sold <span className="text-red-400">{formatNumber(activity.amount!)}</span>{' '}
                        {activity.token.symbol}
                        <span className="text-gray-400"> (${formatNumber(activity.value!)})</span>
                      </span>
                    )}
                    {activity.type === 'create' && (
                      <span>
                        Created new token{' '}
                        <span className="text-purple-400">{activity.token.symbol}</span>
                      </span>
                    )}
