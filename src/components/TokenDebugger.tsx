...
export default function TokenDebugger() {
  ...
  const loadAndAnalyzeTokens = () => {
    try {
      ...
      const userTokens = storedTokens.filter(
        token => token.walletAddress.toLowerCase() === currentAddress
      );
    ...
    <div className="bg-blue-900/20 border border-blue-500 rounded-lg p-6 mb-6">
        ...
        <div className="bg-gray-800 rounded-lg p-4">
          ...
          <div className="space-y-1 text-sm">
            <p className="text-gray-300">
              Total tokens in storage:{' '}
              <span className="text-yellow-400">{debugInfo.totalTokens}</span>
            </p>
            <p className="text-gray-300">
              Your tokens: <span className="text-blue-400">{debugInfo.userTokens}</span>
            </p>
            <p className="text-gray-300">
              Your approved tokens:{' '}
              <span className="text-green-400">{debugInfo.approvedUserTokens}</span>
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="bg-gray-800 rounded-lg p-4">
          ...
          <div className="space-y-1 text-sm">
            ...
            <p className="text-green-400">
              Approved: {debugInfo.userTokensByStatus?.approved || 0}
            </p>
      ...
      <div className="mt-4 p-3 bg-gray-800 rounded-lg">
        <p className="text-gray-400 text-sm">
          <strong>Debug Tips:</strong> If you see your tokens but they're not showing in Race to
          Liberty, they might be in 'pending' status. Use the "Approve All My Tokens" button to
          test, or ask an admin to approve them properly.
        </p>
      </div>
    </div>
  );
