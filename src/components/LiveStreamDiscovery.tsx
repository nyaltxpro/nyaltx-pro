...
export default function LiveStreamDiscovery({ className = '' }: LiveStreamDiscoveryProps) {
      ...
      switch (message.type) {
        ...
        case 'stream_updated':
          setActiveStreams(prev =>
            prev.map(stream => (stream.id === message.streamId ? message.data : stream))
          );
          break;
      }
    };
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-b from-gray-900 to-black ${className}`}>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-400">
                Watch live crypto content • {activeStreams.length} streams •{' '}
                {activeStreams.reduce((sum, s) => sum + s.viewerCount, 0).toLocaleString()} viewers
              </p>
            </div>
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center gap-3 text-xs text-gray-500">
                    <span className="px-2 py-1 bg-gray-700 rounded-full">
                      {categories
                        .find(c => c.id === stream.category)
                        ?.label.replace(/[^\w\s]/gi, '') || stream.category}
                    </span>
      ...
      {/* Stream Viewer Modal */}
      {selectedStream && (
        <LiveStreamViewer streamData={selectedStream} onClose={() => setSelectedStream(null)} />
      )}

      {/* Create Stream Modal */}
      {showCreateModal && <PumpFunLiveStream onClose={() => setShowCreateModal(false)} />}
    </div>
  );
}
