...
export const useCoinGeckoSearch = () => {
  ...
  const searchCoinGecko = useCallback(
    async (query: string, retries = 2) => {
      ...
      for (let attempt = 0; attempt <= retries; attempt++) {
        try {
          ...
          console.log(
            `🔍 Attempt ${attempt + 1}: Using ${attempt === 0 ? 'multi-source' : 'hybrid'} search`
          );
          ...
          if (response.ok) {
            ...
            console.log(
              `✅ ${attempt === 0 ? 'Multi-source' : 'Hybrid'} API found ${coinCount} coins for "${query}" (${successRate}% with addresses)`
            );
            ...
            // Cache the results
            dispatch(
              setCoinGeckoSearchResults({ query: normalizedQuery, results: convertedCoins })
            );
          ...
          if (error.name === 'AbortError') {
            ...
            console.error(
              `❌ Error in search attempt ${attempt + 1}/${retries + 1}:`,
              error.message
            );
          }
