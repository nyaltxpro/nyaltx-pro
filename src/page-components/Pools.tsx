...
const PoolsPageClient = dynamic(() => Promise.resolve(PoolsPageComponent), {
    ssr: false,
    loading: () => <div className="text-gray-400 py-8 text-center">Loading pools...</div>
});
...
function PoolsPageComponent() {
    ...
    } | null>({ key: 'tvl', direction: 'descending' });
    ...
    const toggleFavorite = (id: string) => {
        setPools(
            pools.map(pool => (pool.id === id ? { ...pool, isFavorite: !pool.isFavorite } : pool))
        );
    };
            ...
            <div className="container mx-auto px-4 py-8">
                        ...
                        <table className="min-w-full">
                            <thead>
                                <tr className="bg-gray-800 text-gray-400 text-sm">
                                    ...
                                    >
                                        <div className="flex items-center justify-end">TVL {getSortIcon('tvl')}</div>
                                    </th>
                                    <th
                                        className="px-4 py-3 text-right cursor-pointer"
                                    ...
                                    >
                                        <div className="flex items-center justify-end">APR {getSortIcon('apr')}</div>
                                    </th>
                            ...
                            <tbody>
                                {currentPools.map(pool => (
                                    <tr key={pool.id} className="border-t border-gray-800 hover:bg-gray-800">
                                        ...
                                        <td className="px-4 py-4 text-right font-medium">{formatCurrency(pool.tvl)}</td>
                                        <td className="px-4 py-4 text-right">{formatCurrency(pool.volume24h)}</td>
                                        <td className="px-4 py-4 text-right">{formatCurrency(pool.volume7d)}</td>
                                        <td className="px-4 py-4 text-right">{formatCurrency(pool.fees24h)}</td>
                                        ...
                                        <td className="px-4 py-4 text-center">
                                            <div className="flex justify-center space-x-2">
                                                <button
                                                    className="p-2 bg-blue-900 hover:bg-blue-800 rounded-lg"
                                                    title="Add Liquidity"
                                                >
                    ...
                    {totalPages > 1 && (
                        <div className="px-4 py-3 flex items-center justify-between border-t border-gray-800">
                            <div className="text-sm text-gray-400">
                                Showing {indexOfFirstPool + 1} to {Math.min(indexOfLastPool, sortedPools.length)} of{' '}
                                {sortedPools.length} pools
                            </div>
                ...
                <div className="bg-[#0f1923] rounded-lg p-6 mb-8">
                    ...
                    <div className="text-gray-400">
                        <p className="mb-4">
                            The Pool Explorer allows you to discover and analyze liquidity pools across multiple
                            networks and protocols. Find the best opportunities for providing liquidity and
                            earning fees.
                        </p>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                            <div className="bg-gray-800 p-4 rounded-lg">
                                ...
                                <p className="text-sm">
                                    Total Value Locked (TVL) represents the sum of all assets deposited in a liquidity
                                    pool. Higher TVL generally indicates more stable prices with less slippage.
                                </p>
                            </div>
                            <div className="bg-gray-800 p-4 rounded-lg">
                                <h3 className="font-medium mb-2 flex items-center">
                                ...
                                <p className="text-sm">
                                    Annual Percentage Rate (APR) shows the estimated yearly return from providing
                                    liquidity, including trading fees and any additional rewards from the protocol.
                                </p>
                            </div>
                        </div>
                    </div>
