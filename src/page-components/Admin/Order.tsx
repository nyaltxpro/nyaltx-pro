...
const AdminOrdersComponent = () => {
    ...
    useEffect(() => {
        ...
        fetch('/api/admin/orders/subscriptions')
            .then(async r => {
            ...
            <section className="rounded-xl border border-gray-800 p-4">
                ...
                <form onSubmit={submitOnchain} className="grid grid-cols-1 md:grid-cols-6 gap-3 mb-4">
                    ...
                    <select
                        className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        value={form.tierId}
                        onChange={e => setForm({ ...form, tierId: e.target.value as any })}
                    >
            ...
            <section className="rounded-xl border border-gray-800 p-4">
                ...
                <form onSubmit={submitSubscription} className="grid grid-cols-1 md:grid-cols-6 gap-3 mb-4">
                    ...
                    <select
                        ...
                        onChange={e => setSubForm({ ...subForm, paymentMethod: e.target.value as any })}
                    >
                    ...
                    <div className="md:col-span-6 flex items-center gap-2">
                        <button
                            disabled={subBusy}
                            className="rounded bg-blue-600 hover:bg-blue-500 px-3 py-1 text-sm"
                        >
                            Save Subscription
                        </button>
                                ...
                                {subscriptionOrders.map(s => (
                                    ...
                                    >
                                        ...
                                        <td className="px-6 py-4">
                                            <span
                                                className={`px-2 py-1 rounded text-xs ${s.status === 'active'
                                        ...
                                        <td className="px-6 py-4">
                                            {s.refundStatus ? (
                                                <span
                                                    className={`px-2 py-1 rounded text-xs ${s.refundStatus === 'completed'
                                                        ? 'bg-green-900 text-green-300'
                                                        : s.refundStatus === 'processing'
                                                            ? 'bg-yellow-900 text-yellow-300'
                                                            : s.refundStatus === 'failed'
                                                                ? 'bg-red-900 text-red-300'
                                                                : s.refundStatus === 'requested'
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="bg-yellow-900 border border-yellow-600 rounded p-3">
                                <p className="text-yellow-200 text-sm">
                                    ⚠️ This will process a refund for the specified amount. This action cannot be
                                    undone.
                                </p>
                            </div>
                        </div>
