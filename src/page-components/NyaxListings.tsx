...
import { FaEthereum, FaExternalLinkAlt, FaGlobe, FaTelegram, FaTwitter } from 'react-icons/fa';
import { SiBinance } from 'react-icons/si';
import nyaxTokensData from '../../nyax-tokens-data.json';

const NyaxListingsClient = dynamic(() => Promise.resolve(NyaxListingsComponent), {
    ssr: true,
});
...
const NyaxListingsComponent: React.FC = () => {
        ...
        <div className="min-h-screen bg-[#0b1217] text-white">
            ...
            <div className="bg-gradient-to-r from-cyan-600 to-blue-600 py-12">
                <div className="container mx-auto px-4">
                    ...
                    >
                        <h1 className="text-4xl md:text-6xl font-bold mb-4">NYAX LISTINGS</h1>
                        ...
                        <div className="flex flex-wrap justify-center gap-4 text-sm">
                            <div className="bg-white bg-opacity-20 px-4 py-2 rounded-full">
                                <span className="font-semibold">
                                    {tokens.filter(t => t.network === 'Ethereum').length}
                                </span>{' '}
                                Ethereum
                            </div>
                            <div className="bg-white bg-opacity-20 px-4 py-2 rounded-full">
                                <span className="font-semibold">
                                    {tokens.filter(t => t.network === 'BSC').length}
                                </span>{' '}
                                BSC
                            </div>
                        </div>
                            ...
                            <div className="flex items-start justify-between mb-4">
                                <div className="flex items-center gap-3">
                                    <div className="w-12 h-12 rounded-full overflow-hidden bg-gray-700 flex-shrink-0">
                                        ...
                                        <div
                                            className={`w-full h-full bg-gradient-to-r from-cyan-500 to-blue-500 flex items-center justify-center text-white font-bold ${token.logo ? 'hidden' : ''}`}
                                        >
                                            {token.symbol?.[0] || '?'}
                                        </div>
                                    </div>
...
export default function NyaltzListingsPage() {
    return <NyaxListingsClient />;
}
