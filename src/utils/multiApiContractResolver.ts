  ...
  async searchTokens(query: string): Promise<SearchResult> {
    try {
      ...
      const response = await fetch(
        `${this.baseUrl}/coins/ethereum:0x0000000000000000000000000000000000000000`,
        {
          headers: {
  ...
  async searchTokens(query: string): Promise<SearchResult> {
    try {
      ...
      for (const chainId of chains) {
        try {
          const response = await fetch(
            `${this.baseUrl}/${chainId}/search?query=${encodeURIComponent(query)}`,
            {
              headers: {
                Accept: 'application/json',
              },
...
export class MultiApiContractResolver {
          ...
          result.tokens.forEach((token: TokenInfo) => {
            ...
            if (!seenSymbols.has(symbolKey)) {
              ...
              if (existingToken) {
                ...
                if (
                  token.primaryChain &&
                  (!existingToken.primaryChain ||
                    CHAIN_PRIORITY.indexOf(token.primaryChain) <
                      CHAIN_PRIORITY.indexOf(existingToken.primaryChain))
                ) {
  ...
  async getContractAddresses(symbol: string): Promise<TokenInfo | null> {
    ...
    const exactMatch = results.find(token => token.symbol.toLowerCase() === symbol.toLowerCase());
